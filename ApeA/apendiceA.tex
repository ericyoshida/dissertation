...
analysis of Chapter~\ref{cha:monocular} the more compact Lie-algebra
($\mathfrak{se}(3)$) form of Section~\ref{ap:extrinsic_jac} is used instead.
...
the focal lengths. A small pose perturbation is represented, as in
Section~\ref{sec:pose_perturbations}, by the coordinate vector
$\bm{\xi}=[\bm{\delta\theta}^{\top}\ \bm{\delta t}^{\top}]^{\top}\in\mathbb{R}^{6}$ of an
element of the Lie algebra $\mathfrak{se}(3)$, that is, of the tangent space to
$SE(3)$ at the identity, and to first order --- after truncating the exponential
map as $\exp(\bm{\xi}^{\wedge})\approx\bm{I}_4+\bm{\xi}^{\wedge}$ --- it displaces
