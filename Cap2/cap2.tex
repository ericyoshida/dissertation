...
\begin{equation}
  ...
  \label{eq:ekf_jac}
\end{equation}
be the process and measurement Jacobians, that is, the Jacobians of $\bm{f}$ and
$\bm{h}$ evaluated, respectively, at the previous estimate
$\hat{\bm{x}}_{k-1|k-1}$ and at the predicted state $\hat{\bm{x}}_{k|k-1}$. The EKF
propagates the covariance through these first-order approximations.
...
\end{align}
and $\bm{P}_{k|k} = (\bm{I}-\bm{K}_k\bm{H}_k)\,\bm{P}_{k|k-1}$, where the
innovation $\bm{\nu}_k$ is the vector difference between the measurement and its
prediction and $\bm{S}_k$ is its covariance. Unlike the linear
...
Every quantity in this dissertation is expressed in one of a small number of
reference frames, and much of the technical content consists of moving between
them correctly. This section fixes those frames, the rotation convention that
relates them, and the algebra of rigid-body transforms, including the
representation of an \emph{uncertain} transform that Chapter~\ref{cha:monocular}
requires. The three subsections proceed from the frames themselves, to the
transforms that connect them, to the linearized perturbation of such a transform.
...
\subsection{Pose Perturbations and the Lie Algebra of $SE(3)$}
\label{sec:pose_perturbations}

When the platform pose is uncertain (Chapter~\ref{cha:monocular}), that
uncertainty must be propagated through the geometry, and doing so requires a
\emph{linear} coordinate in which a small change of pose can be represented. The
set $SE(3)$ of rigid-body transforms is not a vector space --- the sum of two
homogeneous transforms is not itself a transform --- so a pose error cannot be
represented by simple addition. It is, however, a \emph{Lie group}: a smooth
manifold that is simultaneously a group under matrix multiplication. The
construction recalled below is standard, and a tutorial treatment is given by
\citen{sola2018microlie}.

A tangent space is defined \emph{at a point} of the manifold, so one does not
speak of ``the'' tangent space of $SE(3)$ without saying where. The tangent space
at the group identity $\bm{I}_4$ is distinguished, because it inherits the group
structure, and is called the \emph{Lie algebra} of $SE(3)$, denoted
$\mathfrak{se}(3)$. Its elements are the $4\times4$ matrices
\begin{equation}
  \bm{\xi}^{\wedge} =
  \begin{bmatrix} [\bm{\delta\theta}]_\times & \bm{\delta t} \\[2pt]
                  \bm{0}_{1\times3} & 0 \end{bmatrix} \in \mathfrak{se}(3),
  \qquad
  [\bm{a}]_\times = \begin{bmatrix} 0 & -a_3 & a_2 \\ a_3 & 0 & -a_1 \\ -a_2 & a_1 & 0 \end{bmatrix},
  \label{eq:se3_algebra}
\end{equation}
where $[\,\cdot\,]_\times:\mathbb{R}^3\to\mathfrak{so}(3)$ is the skew-symmetric
(cross-product) matrix, itself the Lie algebra of the rotation group $SO(3)$.
Such a matrix is determined completely by the six real numbers collected in
\begin{equation}
  \bm{\xi} = \begin{bmatrix} \bm{\delta\theta} \\ \bm{\delta t} \end{bmatrix} \in \mathbb{R}^{6},
  \label{eq:se3_coords}
\end{equation}
in which $\bm{\delta\theta}$ holds the three infinitesimal rotation components and
$\bm{\delta t}$ the three translation increments. The \emph{hat} map
$(\cdot)^{\wedge}:\mathbb{R}^{6}\to\mathfrak{se}(3)$ defined by
\eqref{eq:se3_algebra}, and its inverse the \emph{vee} map, are linear bijections,
so $\mathfrak{se}(3)$ is isomorphic to $\mathbb{R}^{6}$ as a vector space. This
isomorphism is what allows a pose uncertainty to be carried by an ordinary
$6\times6$ covariance matrix on $\bm{\xi}$, and it is in this precise sense that
$\bm{\xi}$ is referred to as a pose perturbation: strictly, $\bm{\xi}$ is the
coordinate vector, in $\mathbb{R}^6$, of an element of the Lie algebra, that is,
of the tangent space at the identity, and $\eqref{eq:se3_algebra}$ is a
parameterization of that $4\times4$ matrix rather than the matrix itself.

The Lie algebra is mapped back onto the group by the exponential map
$\exp:\mathfrak{se}(3)\to SE(3)$, which here is the matrix exponential.
A perturbed pose is therefore composed \emph{multiplicatively} rather than
additively. Adopting the \emph{left} perturbation convention used in
Chapter~\ref{cha:monocular}, in which the error acts in the target (optical)
frame,
\begin{equation}
  \tilde{\bm{T}} = \exp\!\big(\bm{\xi}^{\wedge}\big)\,\bm{T},
  \label{eq:se3_perturb_group}
\end{equation}
with $\bm{T}\in SE(3)$ the nominal pose and $\bm{\xi}$ small. Truncating the
exponential series after its first-order term,
\begin{equation}
  \exp\!\big(\bm{\xi}^{\wedge}\big)
  = \bm{I}_4 + \bm{\xi}^{\wedge} + \tfrac{1}{2}\big(\bm{\xi}^{\wedge}\big)^{2} + \cdots
  \;\approx\; \bm{I}_4 + \bm{\xi}^{\wedge},
  \label{eq:se3_exp_approx}
\end{equation}
and applying \eqref{eq:se3_perturb_group} to a point $\bm{p}^{\mathcal{O}}$
expressed in the camera frame gives, in homogeneous coordinates,
$\tilde{\bm{p}}^{\mathcal{O}} \approx \bm{p}^{\mathcal{O}}
 + [\bm{\delta\theta}]_\times\bm{p}^{\mathcal{O}} + \bm{\delta t}$.
Applying the identity $[\bm{a}]_\times\bm{b}=-[\bm{b}]_\times\bm{a}$ to move the
unknown $\bm{\delta\theta}$ to the right of the cross product puts the
displacement in a form that is linear in $\bm{\xi}$,
\begin{equation}
  \delta\bm{p}^{\mathcal{O}} \;\approx\; -[\bm{p}^{\mathcal{O}}]_\times\,\bm{\delta\theta} + \bm{\delta t}
  \;=\; \underbrace{\big[\,-[\bm{p}^{\mathcal{O}}]_\times \;\; \bm{I}_3\,\big]}_{3\times 6}\;\bm{\xi},
  \label{eq:se3_pert}
\end{equation}
whose truncation error is $O(\lVert\bm{\xi}\rVert^{2})$ and is therefore
negligible for the small navigation-pose errors of interest here. Equation
\eqref{eq:se3_pert} is the $3\times6$ block that the extrinsic Jacobian of
Chapter~\ref{cha:monocular} composes with the projection Jacobian in order to
carry the platform-pose covariance into the filter's innovation.

\section{Motion Models}

A recursive filter needs a model of how the target evolves between measurements.
Because the obstacles considered here are uncooperative, their control inputs are
unknown and their motion must be described kinematically, with the unmodeled
acceleration absorbed into a stochastic term. The two subsections below introduce
the kinematic model adopted throughout and the process-noise covariance that goes
with it; the rotational invariance of that covariance turns out to be the pivotal
hypothesis of the equivalence result in Chapter~\ref{cha:tracking}.
...
Modeling the unknown acceleration as a continuous white noise of intensity
...
The filter observes the target only through a sensor, and this dissertation uses
two quite different ones. A depth or fused camera--LiDAR sensor reports a metric
range and bearing, whereas a monocular camera reports only a pixel. Each leads to
a different measurement equation, a different Jacobian, and a different noise
model, and both are stated here so that Chapters~\ref{cha:tracking}
and~\ref{cha:monocular} can invoke them without re-deriving them.
...
\subsection{Accuracy}
Accuracy is summarized by the Root-Mean-Square Error (RMSE) of the position
estimate at time step $k$, averaged over $M$ Monte~Carlo runs:
\begin{equation}
  \mathrm{RMSE}(k)=\left(\frac1M\sum_{i=1}^{M}\big\lVert \bm{p}_k^{(i)}-\hat{\bm{p}}_k^{(i)}\big\rVert^2\right)^{1/2},
  \label{eq:rmse}
\end{equation}
where $M$ is the number of Monte~Carlo runs, $\bm{p}_k^{(i)}$ is the true target
position at step $k$ in run $i$, and $\hat{\bm{p}}_k^{(i)}$ is the corresponding
estimate.
...
\begin{equation}
  ...
  \label{eq:pcrb}
\end{equation}
where $\bm{J}_k$ is the Fisher information matrix. The inverse of the Fisher
information lower-bounds the error covariance of any unbiased estimator: the
covariance minus $\bm{J}_k^{-1}$ is positive semidefinite,
$\mathbb{E}[(\bm{x}_k-\hat{\bm{x}}_k)(\bm{x}_k-\hat{\bm{x}}_k)^{\top}]\succeq\bm{J}_k^{-1}$.
In particular, the variance of each state component is bounded below by the
corresponding diagonal entry of $\bm{J}_k^{-1}$ \cite{tichavsky1998}. This bound is used to assess near-optimality of the
...
proposal stage gives YOLO the real-time throughput that an onboard aerial system
needs.
...
Figure~\ref{fig:yolo_arch} summarizes the resulting one-stage architecture.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{Cap2/yolo_architecture.pdf}
  \caption{One-stage YOLO detection architecture used in this work. A single
  forward pass through a cross-stage-partial backbone, a path-aggregation
  feature-pyramid neck, and an anchor-free decoupled head regresses all boxes and
  class scores; removing the separate region-proposal stage of two-stage detectors
  yields the real-time throughput needed onboard.}
  \label{fig:yolo_arch}
\end{figure}
...
Figure~\ref{fig:pointnet_arch} shows this construction.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{Cap2/pointnet_architecture.pdf}
  \caption{PointNet architecture. Each of the $n$ unordered input points is mapped
  independently by shared multilayer perceptrons, and the per-point features are
  aggregated by a symmetric max-pooling into a global descriptor; this symmetric
  pooling makes the network invariant to the ordering of the points. Small
  learned ``T-Net'' alignments canonicalize the input and feature spaces. The
  global feature feeds a classification head or, concatenated back to the per-point
  features, a segmentation head.}
...
are regressed with a smooth-$L_1$ (Huber) loss. Both losses reappear in
Eq.~\eqref{eq:pn_loss}, which trains the three-dimensional detector of
Chapter~\ref{cha:airsim}.

\section*{Summary and Road Map}
\addcontentsline{toc}{section}{Summary and Road Map}

This chapter has assembled the two toolkits on which the rest of the dissertation
draws, and it is worth recording where each of its results is put to work, so that
the reader can return here for a specific item rather than for the chapter as a
whole. On the estimation side, the Kalman and extended Kalman recursions
\eqref{eq:kf_predict}--\eqref{eq:ekf_gain} are instantiated twice, in the
range--bearing tracker of Chapter~\ref{cha:tracking} and in the monocular tracker
of Chapter~\ref{cha:monocular}; the frames and rotation conventions of
Section~\ref{sec:pose_perturbations} and Eq.~\eqref{eq:euler} fix the geometry
used throughout, and the $SE(3)$ perturbation \eqref{eq:se3_pert} is the
foundation of the extrinsic Jacobian that Chapter~\ref{cha:monocular} derives;
the constant-velocity model \eqref{eq:cv} and its process noise \eqref{eq:cwna}
are the motion model of both trackers, and the rotational invariance of
\eqref{eq:cwna} is precisely the hypothesis on which the equivalence result of
Chapter~\ref{cha:tracking} turns; the spherical measurement \eqref{eq:spherical}
with its range-dependent noise serves Chapters~\ref{cha:tracking}
and~\ref{cha:airsim}, while the pinhole projection \eqref{eq:pinhole} serves
Chapter~\ref{cha:monocular}; and the metrics of Section~\ref{sec:detmetrics},
together with the NEES, NIS, and posterior Cram\'er--Rao definitions that precede
them, are the yardsticks by which every experimental chapter is judged. On the
learning side, the YOLO detector, PointNet and PointNet\texttt{++}, the
single-stage voxel family, and the focal and smooth-$L_1$ losses are the
components that Chapter~\ref{cha:airsim} trains and compares.

With this common vocabulary fixed, the next chapter turns from tools to the
literature: it reviews, under a systematic protocol, what has been built with
these components for aerial perception, and establishes the specific gap that this
