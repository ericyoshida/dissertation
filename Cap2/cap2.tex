...
uncertainty must be propagated through the geometry. A small perturbation of the
pose is described by a six-vector
$\bm{\xi}=[\bm{\delta\theta}^{\top}\;\bm{\delta t}^{\top}]^{\top}$ in the tangent
space of $SE(3)$, where $\bm{\delta\theta}$ collects the three infinitesimal
rotation angles and $\bm{\delta t}$ the three translation increments. To first
order, this perturbation displaces a point expressed in the camera frame by
\begin{equation}
