...
sampled over $12$~yaw, $3$~pitch and $3$~roll orientations at five ranges,
$1$--$5$ drones per frame). The point-cloud networks are trained on the collected
multimodal set ($3$ scenes $\times\,1{,}000$ frames). Evaluation uses a
campaign of $60$ video sequences, disjoint from the training data of the
two-dimensional and frustum stages, ($20$ per environment, $50$--$92$ frames each,
median $72$, captured at $2.7$--$5.1$~Hz) that randomizes the range regime (near $18$--$32$~m, mid
...
The single-stage voxel networks, which have no separate training corpus, are
instead trained on this campaign under an $80/20$ split by sequence, every fifth
sequence in alphabetical order being held out ($12$ of the $60$, four per
environment); the held-out figures are discussed in Chapter~\ref{cha:airsim}.
...
\begin{table}[H]
  \centering
  \footnotesize
  \caption{Point-cloud detector training. All use Adam \cite{kingma2015adam} ($10^{-3}$, weight decay
...
$\sigma_{\mathrm{pose}}\approx10^{-3}$, marginally on the conservative side of the
value at which the mean NIS crosses the consistent value $\bar\eta=2$
(Section~\ref{sec:mono_consistency}). The first detection is initialized by ray--plane
