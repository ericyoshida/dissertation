...
\begin{table}[H]
  ...
  \begin{tabular}{@{}lll@{}}
    \toprule
    Setting & Synthetic (production) & Real-urban (reference) \\
    \midrule
    Backbone        & YOLOv11s ($\approx 10$~M) & YOLOv11s \\
    Input size      & $1280$~px & $640$~px \\
    Epochs          & $80$ & $100$ \\
    Batch           & $16$ & $16$ \\
    Patience        & default & $30$ \\
    Augmentation    & mosaic, HSV, scale & mosaic, HSV, scale \\
    Result          & $\mathrm{mAP}_{50}=0.994$ & $\mathrm{mAP}_{50}=0.731$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}lll@{}}
    ...
    Frustum PointNet\texttt{++}
      ...
      & Focal ($\alpha{=}0.5,\gamma{=}2$) $+\,5\,\mathrm{SL}_1(\text{ctr})+\mathrm{SL}_1(\text{size})$; $30$ ep., batch $8$ \\
    T-Net segmentation
      & $+$ per-point fg/bg head ($1088{\to}256{\to}128{\to}1$)
      & $+\,\mathrm{BCE}(\text{seg})$; $20$ ep., batch $32$ \\
    Frustum ConvNet
      & $16$ depth slices, per-slice mini-PointNet, $1$D conv
      & same heads; $30$ ep., batch $8$ \\
    PointPillars (PFN)
      & BEV $1.25$~m grid, per-pillar mini-PointNet $[64,64]$, 2D CNN
      & center heatmap (focal) $+$ box reg.; $15$ ep., batch $8$ \\
    Dense 3D voxel
...
The dissertation source is maintained at
\url{https://github.com/ericyoshida/dissertation}. The supporting code (the AirSim
dataset-generation and detection pipeline, and the TU~Berlin monocular tracker)
and the datasets are kept in separate repositories; their public release is
planned, and the links will be provided in the final version.
% TODO-REV: replace the line above with the actual public repository URLs (and a Zenodo DOI for the dataset, if released).
